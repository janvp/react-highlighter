...
var Highlighter = React.createClass({displayName: "Highlighter",
  ...
  render: function() {
    var props = blacklist(this.props, 'search', 'caseSensitive', 'matchElement', 'matchClass', 'matchStyle', 'rootElement');

    return React.createElement(this.props.rootElement, props, this.renderChildren(this));
  },
     ...
     *   The element whose children will be rendered.
     *
     * @return {Array}
     *   An array with the children of the element.
     */
  renderChildren: function(element) {
    return React.Children.map(element.props.children, function(highlighter) {
      return function(child) {
        if (React.isValidElement(child)) {
          return React.cloneElement(child, [], highlighter.renderChildren(child));
        } else {
          return highlighter.renderElement(child);
        }
      }
    }(this));
  },
    ...
    if (this.isScalar(subject) && this.hasSearch()) {
  ...
  /**
   * Determine if the body of text is a valid type for processing.
   *
   * @param {string} subject
   *   The body of text that will be searched for highlighted words.
   *
   * @return {Boolean}
   */
  isScalar: function(subject) {
    return (/string|number|boolean/).test(typeof subject);
  },
